...
extern void FUN_NAME_TST(int m, int n, float *src, int rs_s, int cs_s,
...
void time_function_under_test(int num_trials, int num_runs_per_trial,
                              ...
                              int m, int n, float *src, int rs_s, int cs_s,
                              float *dst, int rs_d, int cs_d) {
    ...
    for (int trial = 0; trial < num_trials; ++trial) {
        ...
        */
        ...
        for (int runs = 0; runs < num_runs_per_trial; ++runs) {
            FUN_NAME_TST(m, n, src, rs_s, cs_s, dst, rs_d, cs_d);
        }
...
int main(int argc, char *argv[]) {
    ...
    for (int p = min_size; p < max_size; p += step_size) {
        ...
        // This is overkill
        int64_t buff_size_src = m * n * rs_src * cs_src;
        int64_t buff_size_dst = m * n * rs_dst * cs_dst;
        ...
        // run the test
        time_function_under_test(num_trials, num_runs_per_trial, results,

                                 m, n, src_tst, rs_src, cs_src, dst_tst, rs_dst,
                                 cs_dst);
